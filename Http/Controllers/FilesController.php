...
class FilesController extends Controller
{
     ...
     */
    public function handleQuestionUpload(Request $request)
    {
        // Validate the request
        $request->validate([
            'file' => 'required|max:20480|mimes:xls,xlsx,pdf,jpeg,svg,jpg,png', // Adjust max size to bytes (20480 = 20MB)
        ]);
    ...
    public function handleAnswerUpload(Request $request)
    {
        // Validate the request
        $request->validate([
            'file' => 'required|max:20480|mimes:xls,xlsx,pdf,jpeg,svg,jpg,png', // Adjust max size to bytes (20480 = 20MB)
    ...
    public function handleSchoolUpload(Request $request)
    {
        // Validate the request
        $request->validate([
            'file' => 'required|max:20480|mimes:xls,xlsx,pdf,jpeg,svg,jpg,png', // Adjust max size to bytes (20480 = 20MB)
