...
public class Server implements CommandLineRunner {
   ...
   private final ConcurrentHashMap<Socket, Boolean> isRepresentativeStatus; // Track if the user is a representative
   ...
   public Server(
           ...
           AccParticipantHandler accParticipantHandler,
           RejParticipantHandler rejParticipantHandler,
      ...
      this.isRepresentativeStatus = new ConcurrentHashMap<>();
   ...
   public void run(String... args) throws Exception {
      try (ServerSocket serverSocket = new ServerSocket(PORT)) {
         ...
         while (true) {
            ...
            isRepresentativeStatus.put(clientSocket, false);
   ...
   private String processCommand(Socket clientSocket, String command) {
      ...
      Boolean isRepresentative = isRepresentativeStatus.get(clientSocket); // Check if client is a representative
      if (isLoggedIn == null || (!isLoggedIn && !commandType.equalsIgnoreCase("Login--") && !commandType.equalsIgnoreCase("Register"))) {
         ...
         if (loginResponse.contains("Hi") || loginResponse.contains("Representative")) {
            clientLoginStatus.put(clientSocket, true);
            isRepresentativeStatus.put(clientSocket, login.isRepresentative(tokens));
      ...
      switch (commandType) {
         ...
         case "Login--":
            ...
            if (loginResponse.contains("Hi")) {
               ...
               isRepresentativeStatus.put(clientSocket, login.isRepresentative(tokens)); // Update representative status
            }
            return loginResponse;
         case "Delete":
            if (isLoggedIn) {
         ...
         case "confirm":
            if (isLoggedIn && isRepresentative) {
               return accParticipantHandler.handleVerification(tokens);
            } else {
               return "ERROR: You must be a logged-in representative to use the 'confirm' command.";
            }
         case "viewApplicants":
            if (isRepresentative) {
               return participantHandler.getAllParticipants();
            } else {
               return "ERROR: You must be a logged-in representative to use the 'viewApplicants' command.";
            }
         case "viewApplicants:Accepted":
            if (isLoggedIn && isRepresentative) {
               return accParticipantHandler.getAllAcceptedParticipants();
            } else {
               return "ERROR: You must be a logged-in representative to use the 'viewApplicants:Accepted' command.";
            }
         case "viewApplicants:Rejected":
            if (isLoggedIn && isRepresentative) {
               return rejParticipantHandler.getAllRejectedParticipants();
            } else {
               return "ERROR: You must be a logged-in representative to use the 'viewApplicants:Rejected' command.";
            }
         case "viewChallenges":
            if (isLoggedIn) {
